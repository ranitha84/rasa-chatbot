...
class Zomato:
    ...
    def get_loc_cus_price_results(self, location, price, cuisine):
        location_detail = self.get_location(location, 1)
        ...
        cuisines_dict = {'bakery': 5, 'chinese': 25, 'cafe': 30, 'italian': 55, 'biryani': 7, 'north indian': 50,
                         'south indian': 85}
        range = [0, 20, 40, 60, 80]
        columns = ['restaurant_name', 'restaurant_rating', 'restaurant_address', 'budget_for2people',
                   'restaurant_photo', 'restaurant_url']
        data = []
        response = "no results"
        restaurant_df = pd.DataFrame()
        results = self.restaurant_search("", lat, lon, str(cuisines_dict.get(cuisine)), limit=80)
        d1 = json.loads(results)
        if d1['results_found'] > 0:
            for x in d1['restaurants']:
                data.append({
                    'restaurant_name': x['restaurant']['name'],
                    ...
                    'restaurant_url': x['restaurant']['url']
                })
        if len(data) > 0:
            df1 = pd.DataFrame(data, columns=columns)
            if price:
                df1['budget'] = df1.apply(lambda row: utility.group_by_budget(row), axis=1)

                # sorting by review & filter by budget
                restaurant_df = df1[(df1.budget == price)]
            else:
                restaurant_df = df1
